...
func NewTCPListener(address string, port uint32, ch chan *net.TCPConn) (*TCPListener, error) {
	...
	log.Infof("listening on %v, %v", addr, proto)
	...
	go func() error {
		for {
			...
			log.Infof("Accepted connection %v", conn)
			ch <- conn
		}
	}()
	return &TCPListener{
		...
		passConn := func(conn *net.TCPConn) {
			...
			if found {
				if peer.fsm.adminState != ADMIN_STATE_UP {
					...
					}).Infof("New connection for non admin-state-up peer")
					conn.Close()
					return
				}
				...
				log.WithFields(log.Fields{
					"Topic": "Peer",
				}).Infof("Accepted a new passive connection from:%s", remoteAddr)
				peer.PassConn(conn)
			} else if pg := server.matchLongestDynamicNeighborPrefix(remoteAddr); pg != nil {
				log.WithFields(log.Fields{
					"Topic": "Peer",
				}).Infof("Accepted a new dynamic neighbor from:%s", remoteAddr)
	...
	if _, y := peer.fsm.rfMap[bgp.RF_RTC_UC]; y && path.GetRouteFamily() != bgp.RF_RTC_UC {
		...
		if ignore {
			log.WithFields(log.Fields{
				...
				"Key":   peer.ID(),
				"Data":  path,
			}).Info("Filtered by Route Target Constraint, ignore")
			return nil
		}
	}
		...
		if !path.IsLocal() {
			...
			if id := path.GetOriginatorID(); peer.fsm.gConf.Config.RouterId == id.String() {
				log.WithFields(log.Fields{
					...
					"OriginatorID": id,
					"Data":         path,
				}).Info("Originator ID is mine, ignore")
				return nil
			}
				...
				for _, clusterID := range path.GetClusterList() {
					if clusterID.Equal(peer.fsm.peerInfo.RouteReflectorClusterID) {
						log.WithFields(log.Fields{
							...
							"ClusterID": clusterID,
							"Data":      path,
						}).Info("cluster list path attribute has local cluster id, ignore")
						return nil
					}
				}
				ignore = false
	...
	if path != nil && path.GetRouteFamily() == bgp.RF_RTC_UC && !path.IsWithdraw {
		...
		if path.IsLocal() && path == old {
			log.WithFields(log.Fields{
				...
				"Path":  path,
			}).Info("given rtm nlri is already sent, skipping to advertise")
			return nil
		}
...
func (s *BgpServer) processOutgoingPaths(peer *Peer, paths, olds []*table.Path) []*table.Path {
	...
	if peer.fsm.pConf.GracefulRestart.State.LocalRestarting {
		log.WithFields(log.Fields{
			"Topic": "Peer",
			"Key":   peer.fsm.pConf.State.NeighborAddress,
		}).Info("now syncing, suppress sending updates")
		return nil
	}
			...
			if !peer.fsm.pConf.GracefulRestart.State.LocalRestarting {
				...
				if allEnd {
					...
					log.WithFields(log.Fields{
						"Topic": "Peer",
						"Key":   peer.ID(),
					}).Infof("Now syncing, suppress sending updates. start deferral timer(%d)", deferral)
			...
			if len(eor) > 0 {
				...
				if peer.fsm.pConf.GracefulRestart.State.PeerRestarting {
					if peer.recvedAllEOR() {
						...
						log.WithFields(log.Fields{
							"Topic": "Peer",
							"Key":   peer.fsm.pConf.State.NeighborAddress,
						}).Infof("withdraw %d stale routes", len(pathList))
						server.propagateUpdate(peer, pathList)
					}
				...
				if c := peer.fsm.pConf.GetAfiSafi(bgp.RF_RTC_UC); rtc && c != nil && c.RouteTargetMembership.Config.DeferralTime > 0 {
					log.WithFields(log.Fields{
						"Topic": "Peer",
						"Key":   peer.ID(),
					}).Info("received route-target eor. flash non-route-target NLRIs")
...
func (server *BgpServer) addNeighbor(c *config.Neighbor) error {
	...
	log.Infof("Peer %v using group conf: %v", addr, pgConf)
	...
	log.Infof("Successfully added peer neighbor %v", peer)
	return nil
}
